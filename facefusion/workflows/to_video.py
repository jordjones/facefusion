...
def process_disk_frames() -> ErrorCode:
	...
	if not temp_frame_set:
		logger.error(translator.get('temp_frames_not_found'), __name__)
		return 1

	frame_items : List[Tuple[int, str]] = [ (frame_number, temp_frame_set[frame_number]) for frame_number in sorted(temp_frame_set) ]
	chunk_start, chunk_end = _chunk_range_from_env()

	if chunk_start is None:
		chunk_size = state_manager.get_item('chunk_size_frames') or 0
		if chunk_size > 0 and len(frame_items) > chunk_size:
			job_id = state_manager.get_item('job_id')
			if job_id:
				step_index = state_manager.get_item('step_index') or 0
				temp_frame_paths = [ temp_frame_path for _, temp_frame_path in frame_items ]
		...
		frame_items = frame_items[chunk_start:chunk_end]
	...
	with tqdm(total = len(frame_items), desc = translator.get('processing'), unit = 'frame', ascii = ' =', disable = state_manager.get_item('log_level') in [ 'warn', 'error' ]) as progress:
		...
		with ThreadPoolExecutor(max_workers = state_manager.get_item('execution_thread_count')) as executor:
			...
			future_to_frame : dict[Future[bool], Tuple[int, str]] = {}

			for frame_number, temp_frame_path in frame_items:
				future = executor.submit(process_disk_frame, temp_frame_path, frame_number)
				futures.append(future)
				future_to_frame[future] = (frame_number, temp_frame_path)
			...
			while futures:
				...
				if is_process_stopping():
					...
					try:
						...
						frame_number, _ = future_to_frame[future]
	...
	if failed_frames and not is_process_stopping():
		logger.info(f'frame_processing_retry: attempting serial retry of {len(failed_frames)} failed frame(s)', __name__)
		recovered : set[int] = set()
		for frame_number, _ in failed_frames:
			temp_frame_path = temp_frame_set.get(frame_number)
			if temp_frame_path is None:
				continue
			try:
				process_disk_frame(temp_frame_path, frame_number)
				recovered.add(frame_number)
			except Exception as exception:
				logger.warn(f'frame_processing_retry_failed: frame={frame_number}: {exception}', __name__)
		if recovered:
			logger.info(f'frame_processing_retry_recovered: {len(recovered)}/{len(failed_frames)} previously-failed frames recovered on retry', __name__)
			failed_frames = [ entry for entry in failed_frames if entry[0] not in recovered ]

	if failed_frames:
		failure_rate = len(failed_frames) / len(frame_items)
		logger.warn(f'frame_processing_summary: {len(failed_frames)}/{len(frame_items)} frames failed ({failure_rate:.1%}); using original frames for those positions', __name__)
